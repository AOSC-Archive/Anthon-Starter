 ...
 *
 * Original version is written by Ulrich Drepper <drepper@gnu.ai.mit.edu>, 1995.
 */
...
/* bits and align information for MD5 algorithm */
#define MD5_DIGEST_BITS 128
#define MD5_DIGEST_ALIGN 4
#define MD5_DIGEST_HEX_BYTES (MD5_DIGEST_BITS / 4)
#define MD5_DIGEST_BIN_BYTES (MD5_DIGEST_BITS / 8)
...
#define UNALIGNED_P(p) ((uintptr_t) (p) % __alignof__ (uint32_t) != 0)
        if (UNALIGNED_P(buffer))
            while (len > 64) {
                md5_process_block(memcpy(ctx->buffer, buffer, 64), 64,
                                  ctx);
...
{
    unsigned char bin_buffer_unaligned[MD5_DIGEST_BIN_BYTES +
                                       MD5_DIGEST_ALIGN];
    unsigned char *bin_buffer =
        ptr_align(bin_buffer_unaligned, MD5_DIGEST_ALIGN);
    ...
    md5_stream(f, bin_buffer);

    for (i = 0; i < MD5_DIGEST_BIN_BYTES; i++) {
        sprintf(rtn, "%02X", bin_buffer[i]);
        rtn += 2;
    }
